<?php $this->load->view('admin/partials/header2'); ?>

<div class="box">
	<div class="box-header with-border">
		<h3 class="box-title"><?=$subjudul?></h3>
		<div class="box-tools pull-right">
			<button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i>
			</button>
		</div>
	</div>
	<div class="box-body">
		<div class="row">
			<div class="form-group col-md-4 text-center">
				<select id="jurusan_filter" class="form-control select2" style="width:100% !important">
					<option value="all">Semua Jurusan</option>
					<?php foreach ($jurusan as $m) :?>
					<option value="<?=$m->kode?>"><?=$m->jurusan?></option>
					<?php endforeach; ?>
				</select>
			</div>
			<div class="form-group col-md-4 text-center">
				<select id="tahun_filter" class="form-control select2" style="width:100% !important">
					<option value="all">Semua Tahun</option>
					<?php foreach ($tahun  as $m) :?>
					<option value="<?=$m->tahun ?>"><?=$m->tahun?>/<?=$m->tahun+1?></option>
					<?php endforeach; ?>
				</select>
			</div>
			<div class="col-md-4">
				<div class="pull-right">
					<button type="button" onclick="reload_ajax()" class="btn btn-block btn-md bg-maroon "><i
							class="fa fa-refresh"></i> Reload</button>
				</div>
			</div>
		</div>

		<div class="table-responsive px-4 pb-3 mt-5" style="border: 0">
			<table id="akun" class="table  table-hover table-bordered margin-top-10 table-responsive">
				<thead>
					<tr>
						<th width="25">No.</th>
						<th>No Pendaftaran</th>
						<th>Nama</th>
						<th>Jurusan dipilih</th>
						<th>Tanggal Daftar</th>
						<th>Bukti Pembayaran</th>
						<th>status</th>
						<th class="text-center">Aksi</th>
					</tr>
				</thead>
				<tfoot>
					<tr>
						<th width="25">No.</th>
						<th>No Pendaftaran</th>
						<th>Nama</th>
						<th>Jurusan dipilih</th>
						<th>Tanggal Daftar</th>
						<th>Bukti Pembayaran</th>
						<th>status</th>
						<th class="text-center">Aksi</th>

					</tr>
				</tfoot>
			</table>
		</div>
	</div>



</div>

<script src="<?=base_url()?>assets/admin/jsku/konfirmasi.js"></script>
<script type="text/javascript">
	$(document).ready(function () {
		$('#jurusan_filter').on('change', function () {
			let jurusan = $(this).val();
			let tahun = $('#tahun_filter').val();
			let src = '<?=base_url()?>pendaftaran/datakonfirmasi';
			let url;

			if (jurusan !== 'all' || tahun !== 'all') {
				let src2 = src + '/' + jurusan + '/' + tahun;
				url = $(this).prop('checked') === true ? src : src2;
			} else {
				url = src;
			}
			table.ajax.url(url).load();
		});

		$('#tahun_filter').on('change', function () {
			let tahun = $(this).val();
			let jurusan = $('#jurusan_filter').val();
			let src = '<?=base_url()?>pendaftaran/datakonfirmasi';
			let url;

			if (tahun !== 'all' || jurusan != 'all') {
				let src2 = src + '/' + jurusan + '/' + tahun;
				url = $(this).prop('checked') === true ? src : src2;
			} else {
				url = src;
			}
			table.ajax.url(url).load();
		});
	});

</script>

<?php $this->load->view('admin/partials/footer2'); ?>